$(function () {
  ...
  $.ajax({
    type: "GET",
    url: "http://t.api.twyxedu.com/api/activity/list",
      ...
      if (res.status === 'succ') {
        res.data.array.forEach((item, index) => {
          ...
          if (item.type == '2') {
            ...
            dataList += '<li>' +
              '  <a target="_blank" class="img-box" href="./detail.html?c=growup&id=' + item.id + '" style="background-image:url(' + item.imgUrl + ')"></a>' +
              '  <div class="new-content">' +
              '    <h2>' +
